...
function Beauty() {
    ...
    const deleteHandel = async (id) => {
        const res = await axios.delete(`http://localhost:3000/Beauty/${id}`)
        console.log(res.data);
    ...
    const editdata = async (id) => {
        const res = await axios.get(`http://localhost:3000/Beauty/${id}`);
        console.log(res.data);
    ...
    const Submithandel = async (e) => {
        e.preventDefault();
        if (validation()) {
            const result = await axios.patch(`http://localhost:3000/Beauty/${formvalue.id}`, formvalue)
            console.log(result); // status = 200 edit
            if (result.status == 200) {
                toast.success("Data Update Success!! ")
                setFormvalue({ ...formvalue, id: "", Prod_name: "", price: "", discount: "", size: "", status: '', url: "" })
            }
        }
    }
                        ...
                        <div className="content">
                            <div className="row">
                                ...
                                <div className="col-sm-8 col-9 text-right d-flex justify-content-end m-b-20 ">
                                    <NavLink to="/beauty_prod" className="btn btn btn-primary btn-rounded float-right"><i className="fa fa-plus" />Add Product</NavLink>
                                </div>
